...
public class WorldChunkManagerMadLands extends WorldChunkManager {
	private GenLayer myGenBiomes;
	private GenLayer myBiomeIndexLayer;
	private BiomeCache myBiomeCache;
	private List<BiomeGenBase> myBiomesToSpawnIn;

	protected WorldChunkManagerMadLands() {
		this.myBiomeCache = new BiomeCache(this);
		this.myBiomesToSpawnIn = new ArrayList<BiomeGenBase>();
		this.myBiomesToSpawnIn.add(ModDimensions.madLandsPlains);
	}

	public WorldChunkManagerMadLands(long seed, WorldType worldtype) {
		this();
		// i changed this to my GenLayerTutorial
		GenLayer[] agenlayer = GenLayerMadLands.makeTheWorld(seed);
		this.myGenBiomes = agenlayer[0];
		this.myBiomeIndexLayer = agenlayer[1];
	}

	public WorldChunkManagerMadLands(World world) {
	...
	public BiomeGenBase getBiomeGenAt(int x, int z) {
		...
		if (biome == null) {
			return ModDimensions.madLandsPlains;
		}
	...
	public BiomeGenBase[] getBiomesForGeneration(
		...
		for (int i = 0; i < par4 * par5; ++i) {
			if (aint[i] >= 0) {
				...
				par1ArrayOfBiomeGenBase[i] = ModDimensions.madLandsPlains;
			}
		}
